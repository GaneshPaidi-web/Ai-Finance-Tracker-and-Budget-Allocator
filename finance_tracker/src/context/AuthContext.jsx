...
export const AuthProvider = ({ children }) => {
  ...
  // Safe helper to parse JSON response or fallback gracefully
  const parseResponse = async (res) => {
    try {
      return await res.json();
    } catch {
      return { message: res.statusText || `Server error (${res.status})` };
    }
  };
  ...
  const resetPassword = async (email, securityAnswer, newPassword) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/forgot-password', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: cleanEmail, securityAnswer, newPassword })
      });

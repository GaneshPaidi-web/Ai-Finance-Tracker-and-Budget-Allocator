...
const readLocalDb = () => {
  try {
    ...
    if (!fs.existsSync(JSON_DB_PATH)) {
      fs.writeFileSync(JSON_DB_PATH, JSON.stringify(initialLocalDb, null, 2), 'utf8');
...
export const initDb = async () => {
  ...
  if (!uri) {
    ...
    useLocalDb = true;
    readLocalDb(); // Ensure folder/file is created
    return;
  }
...
export const createUser = async (userData) => {
  if (!useLocalDb) {
    const newUser = new User({ ...userData, email: userData.email.toLowerCase() });
